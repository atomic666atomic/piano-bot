...
# -*- coding: utf-8 -*-
"""
پست جلسه‌ی آموزشی پیانو برای کانال Alipiano (نسخه‌ی ۴ — طبق راهنمای رسمی برند):
...
3) پست را دقیقاً با «ساختار ۲» راهنمای برند می‌سازد:
   - سه لینک قابل‌کلیک (وب‌سایت رسمی، اسپاتیفای، اپل موزیک) + «📌 جلسه کامل»
   - هرگز URL خام نمایش داده نمی‌شود
...
def brand_footer() -> str:
    """بخش پایانی ثابت پست‌های آموزشی: جداکننده + سه لینک + هشتگ‌ها + امضا."""
    return (
        f"{SEPARATOR}\n{brand_links_block()}\n\n"
        f"{LESSON_HASHTAGS}\n\n{SIGNATURE}"
    )
...
۱) خوش‌آمدگویی گرمِ ۱ تا  خطی که حتماً با «سلام دوست عزیزم 🌿» شروع شود و هدف امروز را کوتاه بگوید.
...
۷) «⏱ برنامه پیشنهادی:» و بعد یک برنامه‌ی ۱۵ تا ۰ دقیقه‌ای که به بخش‌های ۵ تا ۱۰ دقیقه‌ای تقسیم شده و هر بخش با • شروع شود.
...
- بخش پایانی را ننویس: سه لینکِ «وب‌سایت رسمی»، «اسپاتیفای»، «اپل موزیک»، «📌 جلسه کامل»، هشتگ‌ها و امضا را سیستم اضافه می‌کند؛ پس بعد از «⏱ برنامه پیشنهادی» جداکننده‌ی آخر ننویس.
...
def add_self_link(post: str, message_id: int) -> str | None:
    """لینکِ خودِ پست (📌 جلسه کامل) را بعد از خط اپل موزیک اضافه می‌کند.
...
def main() -> None:
    ...
    # هارمون‌کردن خط‌های جداکننده‌ای که AI نوشت (هر نوع خط کشی: ─ - —) تا دقیقاً ۲۰ کاراکتر ─ شوند
    body = re.sub(r"^\s*[-─━—–]{8,}\s*$", SEPARATOR, body, flags=re.MULTILINE)
    # پاک‌کردن فاصله‌های اضافی از ابتدای/اواخر بدنه
    body = body.strip()
